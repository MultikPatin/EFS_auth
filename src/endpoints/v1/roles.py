...
from src.validators.permission import (
    PermissionValidator,
...
async def update_role(
    ...
    """
    await current_user.is_superuser(request)
    role = await roles_service.update(await role_validator.is_exists(role_uuid), body)
    return role
...
async def remove_role(
    ...
    return StringRepresent(code=HTTPStatus.OK, details="Role deleted successfully")
...
async def add_permission_to_role(
    ...
    role_validator: RoleValidator = Depends(get_role_validator),
    permission_validator: PermissionValidator = Depends(get_permission_validator),
...
async def remove_permission_from_role(
    ...
    role_uuid = await role_validator.is_exists(role_uuid)
    permission_uuid = await permission_validator.is_exists(permission_uuid)
    await role_permission_service.remove_permission_for_role(role_uuid, permission_uuid)
